...
function Sonoff() {
    if (!(this instanceof Sonoff))
        return new Sonoff(); emitter.call(this);
}
...
Sonoff.prototype.setTimer = function (device, time, state) {
    ...
    if (!d) {
        ...
        if (self._connected) {
            d.timers = d.timers || [];
            d.timers.push({ enabled: true, type: time.includes('T') ? 'once' : 'repeat', at: time, do: { switch: state ? 'on' : 'off' } });
...
var _initDevice = (self, nic, ssid, pwd, force) => {
    exec("route change 0.0.0.0 mask 0.0.0.0 10.10.7.1", function (err, res) {
        if (err) {
            ...
            }).on('error', (e) => { console.log(`Unable to establish connection to the device: ${e.message}`); });
        }
    });
};
                ...
                if (address.family === 'IPv4' && !address.internal && address.mac == self._nic.mac) {
                    ...
                    var server = https.createServer(options, (req, res) => {
                        ...
                        req.on('data', function (chunk) {
                            ...
                            console.log('REQ | %s', JSON.stringify(body)); res.writeHead(200);
